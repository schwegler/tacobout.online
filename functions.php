...
function tacobout_enable_mastodon_apps_login_redirect( $redirect_to, $requested_redirect_to ) {
	if ( isset( $_REQUEST['action'] ) && 'enable-mastodon-apps-authenticate' === $_REQUEST['action'] ) {
		// 🛡️ Sentinel: Sanitize redirect and block dangerous schemes to prevent XSS/Open Redirect.
		// Cannot use wp_validate_redirect() as it blocks valid external custom app URI schemes (e.g. ivory://)
