...
function tacobout_enable_mastodon_apps_login_redirect( $redirect_to, $requested_redirect_to ) {
	if ( isset( $_REQUEST['action'] ) && 'enable-mastodon-apps-authenticate' === $_REQUEST['action'] ) {
		...
		// Parse the scheme from the sanitized URL.
		$scheme = wp_parse_url( $sanitized_redirect, PHP_URL_SCHEME );

		// Block dangerous schemes to prevent XSS Open Redirect.
		if ( $scheme && in_array( strtolower( $scheme ), array( 'javascript', 'vbscript', 'data' ), true ) ) {
			return $redirect_to; // Fallback to safe default redirect.
		}
