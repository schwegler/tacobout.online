...
## 2024-08-01 - [CSS Animation Performance]
**Learning:** Animating `box-shadow` (e.g., for pulse effects) triggers continuous CPU repaints, causing performance degradation.
**Action:** Replace `box-shadow` animations with a `::before` or `::after` pseudo-element that animates `transform: scale()` and `opacity` to offload the work to the GPU compositor thread.
