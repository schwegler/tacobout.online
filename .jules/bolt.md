...
## 2024-09-05 - [Cache offsetHeight in Scroll Handlers]
**Learning:** Reading layout properties like `offsetHeight` inside high-frequency event handlers (such as `scroll`) causes synchronous layout calculation (layout thrashing) on every frame.
**Action:** Cache these values outside the handler and update them on `resize` to prevent main-thread blocking.
