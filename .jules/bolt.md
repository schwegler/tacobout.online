...
## 2024-07-30 - [Cache offsetHeight in scroll handlers]
**Learning:** Reading layout properties like `offsetHeight` inside high-frequency event handlers (such as `scroll`) causes synchronous layout calculation (reflow/layout thrashing).
**Action:** Cache these values outside the handler and update them on `resize` to prevent layout thrashing.
