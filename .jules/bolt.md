...
## 2024-08-01 - [CSS Box-Shadow Animation Performance]
**Learning:** Animating `box-shadow` (e.g., for pulse effects) triggers continuous CPU repaints, which severely impacts rendering performance.
**Action:** Replace `box-shadow` animations with a `::before` or `::after` pseudo-element that animates `transform: scale()` and `opacity` to offload the work to the GPU compositor thread. Namespace global keyframes to prevent collisions.
