...
## 2024-08-01 - [CSS GPU Animation for Pulse Effects]
**Learning:** Animating `box-shadow` directly (e.g., for pulse effects) triggers continuous CPU repaints.
**Action:** To optimize CSS animation performance, replace `box-shadow` animations with a `::before` or `::after` pseudo-element that animates `transform: scale()` and `opacity` to offload the work to the GPU compositor thread.
