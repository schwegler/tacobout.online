...
## 2024-08-22 - [CSS Box-Shadow Animation Performance]
**Learning:** Animating `box-shadow` triggers continuous CPU repaints.
**Action:** Replace `box-shadow` animations with a `::before` or `::after` pseudo-element that animates `transform: scale()` and `opacity` to offload work to the GPU.
