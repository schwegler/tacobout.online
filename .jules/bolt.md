...
## 2024-07-29 - [Optimizing CSS Animations]
**Learning:** Animating `box-shadow` (e.g., for pulse effects) triggers continuous CPU repaints.
**Action:** Replace `box-shadow` animations with a `::before` or `::after` pseudo-element that animates `transform: scale()` and `opacity` to offload the work to the GPU compositor thread. Namespace the animation name to prevent unintended global style collisions.
