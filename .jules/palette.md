...
## 2026-08-16 - Theme.json Focus Parity
**Learning:** In WordPress `theme.json`, defining `":hover"` styles for interactive elements like links and buttons does not automatically apply them to keyboard focus states, leaving keyboard users without visual feedback.
**Action:** Always add a mirroring `":focus"` pseudo-class object with identical properties whenever `":hover"` is defined in `theme.json` to ensure accessible focus parity.
