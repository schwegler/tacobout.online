...
## 2024-08-19 - Focus Parity in theme.json
**Learning:** In WordPress `theme.json`, explicit focus parity is required for accessibility. When defining `":hover"` styles for interactive elements, a mirroring `":focus"` pseudo-class object with identical properties must be added to ensure keyboard users receive corresponding visual feedback.
**Action:** Always add a `:focus` object with identical properties when defining `:hover` styles in `theme.json`.
