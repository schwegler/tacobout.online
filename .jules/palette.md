...
## 2024-08-06 - Explicit Focus Parity in theme.json
**Learning:** In WordPress `theme.json`, explicit focus parity is required for accessibility. When defining `":hover"` styles for interactive elements like links and buttons, they lack automatic keyboard focus mirroring, leaving keyboard users without corresponding visual feedback.
**Action:** When defining `":hover"` styles for interactive elements in `theme.json`, always add a mirroring `":focus"` pseudo-class object with identical properties to ensure complete visual feedback for keyboard navigation.
