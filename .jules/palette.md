...
## 2024-08-09 - Focus Parity in theme.json
**Learning:** In WordPress `theme.json`, explicit focus parity is required for accessibility. When defining `":hover"` styles for interactive elements, they don't automatically mirror to focus states.
**Action:** Always add a mirroring `":focus"` pseudo-class object with identical properties to `":hover"` in `theme.json` to ensure keyboard users receive corresponding visual feedback.
