...
## 2026-07-02 - Infinite Scroll End Feedback
**Learning:** Infinite scroll lists often fail silently when they reach the end, leaving users wondering if the content is still loading or broken. This is especially problematic for screen reader users who receive no audible feedback.
**Action:** Always provide explicit visual and audible (via `aria-live="polite"`) feedback when an infinite scroll feed reaches its conclusion to assure users that they have seen all available content.
