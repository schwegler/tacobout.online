...
## 2026-08-15 - Explicit Focus Parity in theme.json
**Learning:** In WordPress `theme.json`, explicit focus parity is required for accessibility. When defining `":hover"` styles for interactive elements like links and buttons, they do not automatically apply to focus states.
**Action:** Always add a mirroring `":focus"` pseudo-class object with identical properties alongside `":hover"` to ensure keyboard users receive corresponding visual feedback.
